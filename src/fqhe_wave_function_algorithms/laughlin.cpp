	...
	: FQHE::WaveFunction(wavefunctionData)
{
	...
	if(m_wfData->geometry == _TORUS_)
	{
		...
		m_thetaFuncs = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.5, -0.5, 
		                                          dcmplx(0.0, 0.0), true, 1);
		m_thetaFuncsQh = 0;
		m_thetaFuncsCom = 0;
						    ...
						    //!		must equal nbrPos.
{
    ...
    for(int i=0; i<origNbr; ++i)
    {
	    if(keepIndexes[i] != -1)
	    {
			...
			#if _TEST_MODE_
		    std::cout << "\t filtered (w1+w2+...) " << counter << " is " 
    ...
    for(int a=0; a<m_wfData->torusDegeneracy; ++a)
    {
        ...
        {
            m_thetaFuncsCom[counter] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 
                thetaArg+(double)a/m_wfData->jastrowExponent, thetaArg, -m_uniqueCom[i], 
                true, m_wfData->jastrowExponent);
            ++counter;
        }
    }
	#endif
	...
	const
{
    ...
    {
        std::cout << "ERROR: Laughlin::evaluate_wf(int n, dcmplx *z,int whichState) whichState "
                  << "argument must be between 0 and jastrowExponent-1" << std::endl;
        exit(EXIT_FAILURE);
    }
	dcmplx wfValue = dcmplx(0, 0);
	for(int i=0; i<n; ++i)
	...
	for(int i=0; i<m_wfData->jastrowExponent; ++i)
	{
		wfValue += utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5, 
		    (zcm-laughlinComZeros[i])/(double)Lx,(double)Lx/Ly);
	}
    #endif
    #if _WITH_GENERALISED_THETA_FUNCS_
	double thetaArg = (m_wfData->flux-m_wfData->jastrowExponent)/2.0;
	wfValue += utilities::thetaFunction::GeneralisedJacobi(thetaArg+(double)whichState/m_wfData->jastrowExponent, 
	    thetaArg, ((double)m_wfData->jastrowExponent/Lx)*zcm, (double)m_wfData->jastrowExponent*Ly/Lx);
    #endif
	#if _TEST_MODE_
	...
	const
{
	...
	if(whichState>=m_wfData->jastrowExponent)
	{
		std::cout << "ERROR: Laughlin::evaluate_wf(int n, dcmplx *z,int whichState) "
	...
	for(int j=0; j<nbrQh; ++j)
	{
									    ...
									    //	has not been assigned
		{
			for(int i=0; i<n; ++i)
			{
				wfValue += utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5, (dcmplx(real(latticeConfig[i]),
				            imag(latticeConfig[i]))-zQh[j])/(double)Lx,(double)Ly/Lx);
			}
		}
		else
		{
			for(int i=0; i<n; ++i)
			{
				...
				wfValue += *(m_thetaFuncsQh[currentPosition]->thetaLookUpTable+real(latticeConfig[i])*Ly
				        +imag(latticeConfig[i]));
				#if _TEST_MODE_ == 2
	...
	if(m_thetaFuncsCom != 0)        //  if a table of value is stored, use that
	{
        ...
        while(imag(thetaArgNoQuasiHole)>=t)
	    {
			...
			#if _TEST_MODE_ == 2
				std::cout << "Result with shift: " << xShiftFactor+yShiftFactor
				    +utilities::thetaFunction::GeneralisedJacobi(arg1, arg2, thetaArgWithQuasiHole, t) 
				    << std::endl;				
			#endif
	    }
		#if _TEST_MODE_ == 2
	    ...
	    wfValue += *(m_thetaFuncsCom[whichState*m_nbrCom+currCom]->thetaLookUpTable
	        +(int)real(thetaArgNoQuasiHole*(double)Lx)*Ly*m_wfData->jastrowExponent
	        +(int)imag(thetaArgNoQuasiHole*(double)Lx));
