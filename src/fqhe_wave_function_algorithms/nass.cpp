	...
	:  FQHE::WaveFunction(wavefunctionData)
{
	...
	//	some local variables which will be needed here
	int i,j,k;
	int ktot=0,dperms=0;
	
	//	The number of permutations depends on the number of particles
	
	int permsNumber[10]={2,6,20,70,252,924,3432,12870,48620,184756};
	
	//	Pick permutation number from list:
	...
	for (i=0;i<m_nbrPerms;i++)
	{
		for (j=0;j<m_nOverTwo;j++)
		{
			f_perms>>m_permsList[i*m_nOverTwo+j];
			//	N.B for c++ arrays, we now have to reduce the indicies by 1.
	...
	for (i=0;i<m_nbrPerms-1;i++)
	{
		k=0;
		for (j=0;j<m_wfData->nbr/4;j++)
		{
	...
	for (i=0;i<m_nbrPerms-1;i++)
	{
		for (j=0;j<m_wfData->nbr/4;j++)
		{
			dperms=m_permsList[i*m_nOverTwo+j]-m_permsList[(i+1)*m_nOverTwo+j];
			if (dperms!=0)
			{
	...
	{	
		k=0;
		for (j=m_wfData->nbr/4;j<m_wfData->nbr/2;j++)
		{
			dperms=m_permsList[i*m_nOverTwo+j]-m_permsList[(i+1)*m_nOverTwo+j];
			if (dperms!=0)	k++;
		}
...
dcmplx FQHE::NonAbelianSpinSinglet::EvaluateWfSphere(
	...
	for(k=0,p_currPerm=m_permsList;k<m_nbrPerms/2;k++,p_currPerm+=m_nOverTwo)
	{
		cmplxCtrA=0.0;
		cmplxCtrB=0.0;
	...
	for(i=0,currDiff1=m_zwDiff,currDiff2=m_zwDiff+n/4*m_nOverTwo;i<n/4;i++,currDiff1+=m_nOverTwo,currDiff2+=m_nOverTwo)
	{
        for(j=0;j<n/4;j++)
		{
	...
	p_cmplxCtr1=m_cmplxCtr1;
	p_zPerm_sizeA=m_sizeListA;
	p_zPerm_sizeB=m_sizeListB;
	p_zPerm_indexA1=m_indexA1;
	p_zPerm_indexA2=m_indexA2;
	p_zPerm_indexB1=m_indexB1;
	p_zPerm_indexB2=m_indexB2;
	
	for(zPerm=0;zPerm<m_nbrPerms/2;zPerm++,p_currPerm+=m_nOverTwo,p_zPerm_sizeA++,p_zPerm_sizeB++,p_cmplxCtr1++)
	{
		cmplxCtrC=exp(cmplxCtrB+*(m_cmplxCtr2));
		...
		for(wPerm=0;wPerm<m_nbrPerms-1;wPerm++,p_wPerm_sizeA++,p_wPerm_sizeB++,p_cmplxCtr2++)
		{
				cmplxCtrD=0.0;
		...
		dotProd=*p_cmplxCtr1+log(cmplxCtrC);
