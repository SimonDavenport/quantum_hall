    ...
    const int lzA)                                      //!<    Angular momentum
{
    if(!m_hilbertSpaceBuilt)
    {
	    ...
	    if(nbrColours>1)	
	    {
	        colours[1]=_U1_LABEL_2_;
	    }
	    if(nbrColours>2)	
	    {
	        colours[2]=_U1_LABEL_3_;
	    }
	    if(nbrColours>3)
	    {
	    ...
	    for(unsigned int p=0; p<neutralModePartition.size(); ++p)
	    {
	        ...
	        if(useMajoranaModes)
	        {
                ...
                utilities::Generate_k_r_AdmissiblePartitions(
                    majoranaPartitionList, (lzA-neutralModePartition[p]), 
                    nbrA, 2, 2, (nbrA & 1));	
	        ...
	        for(int b=0;b<nbrBlocks;b++)
	        {
		        if(neutralModePartition[p]-deltaLza[b]>0)
		        {
			        ...
			        utilities::GenerateColouredIntegerPartitions(
			            neutralPartitionList, neutralModePartition[p]-deltaLza[b],
    ...
    if(m_hilbertSpaceBuilt)
    {
		        ...
		        {  
			        ...
			        for(int j=0; j<i ; ++j, ++p_matrix)
			        {
				        *(p_matrix) = p_block[j*blockDim+i];
				        if(printMatrix)		
				        {
				            utilities::cout.SecondaryOutput() << *(p_matrix) << " ";
				        }
			        }
	    ...
	    else
	    {
		    ...
		    for(p_matrix=m_matrixRepresentation, i=0; i<m_dimension; ++i)
		    {
			    ...
			    if(!printMatrix)
			    {
				    utilities::cout.SecondaryOutput() << std::fixed << "\tCALCULATED "
				                                      << 100.0*(i+1)/m_dimension << "%\r";
				    fflush(stdout);
			    }
		    }
	    }
        utilities::cout.SecondaryOutput() << std::endl;
	    utilities::cout.MainOutput() << "\t\tTIME TAKEN TO CONSTRUCT HAMILTONIAN: " 
	                                 << (clock()-timer)/CLOCKS_PER_SEC << " seconds.\n" << std::endl;
...
void HilbertSpace::Diagonalize()
{
    if(m_matrixGenerated && !m_matrixDiagonalized)
    {
	    ...
	    if(m_useBlocks)
	    {
		    ...
		    for(unsigned int i=0; i<m_blockDims.size(); ++i)
		    {
			    utilities::linearAlgebra::DiagonalizeSymmetricMatrix<double>(
			        m_blockRepresentation[i], p_eigenvalues, m_blockDims[i], 'U');
	    ...
	    {
		    utilities::linearAlgebra::DiagonalizeSymmetricMatrix<double>(
		        m_matrixRepresentation, m_eigenvalues, m_dimension, 'U');
	    ...
	    {
		    utilities::cout.SecondaryOutput() << std::setw(10) << "\t" << m_eigenvalues[i] << std::setw(2) 
		                                      << "\t" << m_eigenvalueLabels[i] << std::endl;
	    }
	    utilities::cout.MainOutput() << "\n\t\tTIME TAKEN TO DIAGONALIZE: " << (clock()-timer)/CLOCKS_PER_SEC
	                                 << " seconds.\n" << std::endl;
    ...
    const
{
	...
	fileName.str("");
	fileName << (*optionList)["out-path"].as<std::string>() << "/eigenvalues_" 
	         << (*optionList)["hamiltonian"].as<std::string>() << "_model_n_" 
...
//!
//!	This function outputs eigenvalue data to a text file
//!
