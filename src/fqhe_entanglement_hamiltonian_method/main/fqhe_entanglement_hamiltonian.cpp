...
int main(int argc, char *argv[])
{
    ...
    {
        std::cerr << "\n\tERROR: Invalid statistics option: " << optionList["statistics"].as<int>() 
                  << ". Choose +1 for fermions or -1 for bosons only." << std::endl;
        exit(EXIT_FAILURE);    
    }
	...
	utilities::cout.MainOutput() << utilities::cout.EqualsLine() << std::endl 
	                             << utilities::cout.EqualsLine() << std::endl;
	utilities::cout.MainOutput() << "\n\tPROGRAM OPTIONS" << std::endl;
	utilities::cout.MainOutput() << "\n\t\tOPTIMIZING FOR BLOCKS?\t" 
	                             << (optionList["blocks"].as<bool>() ? "true" : "false") << std::endl;
	utilities::cout.MainOutput() << "\n\t\tNUMBER OF PARTICLES\t" << optionList["nbr-n"].as<int>() << std::endl;
	utilities::cout.MainOutput() << "\n\t\tN_A PARTICLE IN THE CUT\t" 
	                             << optionList["nbr-a"].as<int>() << std::endl;
	if(!multiSector)    
	{
	...
	utilities::cout.MainOutput() << "\n\t\tINCLUDE MAJORANA MODES\t" 
	                             << (optionList["majorana-modes"].as<bool>() ? "YES" : "NO") << std::endl;
	utilities::cout.MainOutput() << "\n\t\tSTATISTICS\t\t" 
	                             << (optionList["statistics"].as<int>() == 1 ? "bosons" :"fermions"  ) << std::endl;
    utilities::cout.MainOutput() << "\n\t\tPARAMETERS FILE\t\t" 
                                 << (optionList["parameters-file"].as<std::string>()) << std::endl;
    ...
    const int lzA)                                      //!<    Angular momentum
{
	...
	ListOfTerms hamiltonian(DubailReadRezai431(alpha, beta, gamma, fillingFactor, lzA, _U1_LABEL_1_,
	                                           (*optionList)["nbr-n"].as<int>()));
    ...
    const int lzA)                                      //!<    Angular momentum
{
	...
	ListOfTerms hamiltonian(DubailReadRezai431(alpha[0], beta[0], gamma[0], 
	    fillingFactor, lzA, _U1_LABEL_1_, (*optionList)["nbr-n"].as<int>()));
	if(nbrCurrents>1)
	{
	    hamiltonian += DubailReadRezai431(alpha[1], beta[1], gamma[1], 
	        fillingFactor, lzA, _U1_LABEL_2_, (*optionList)["nbr-n"].as<int>());
    }
    if(nbrCurrents>2)
	{
	    hamiltonian += DubailReadRezai431(alpha[2], beta[2], gamma[2], 
	        fillingFactor, lzA, _U1_LABEL_3_, (*optionList)["nbr-n"].as<int>());
    }
    if(nbrCurrents>3)
    {
