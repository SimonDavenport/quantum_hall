	...
	: FQHE::WaveFunction(wavefunctionData)
{
	...
	if(m_wfData->geometry == _TORUS_)
	{
		...
		m_thetaFuncs[0] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.5, -0.5, 
		                                             dcmplx(-smallShift, 0.0), false, 1);
		m_thetaFuncs[1] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.5, 0.0, 
		                                             dcmplx(-smallShift, 0.0), false, 1);
		m_thetaFuncs[2] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.0, 0.0,
		                                             dcmplx(-smallShift, 0.0), false, 1);
		m_thetaFuncs[3] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.0, 0.5,
		                                             dcmplx(-smallShift, 0.0), false, 1);
...
{	
	...
	int* keepIndexes = new int[nbrPos];
	...
	for(int i=0; i<nbrPos; ++i)
	{
		if(keepIndexes[i] != -1)
		{
			...
			#if _TEST_MODE_
			std::cout << "\t filtered position " << counter << " is " 
			          << m_uniqueQhPositions[counter] << std::endl;
			#endif
			++counter;
		}
	}
	...
	for(int i=0; i<m_nbrQhPositions; ++i)
	{
		m_thetaFuncsQh[i] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.5, -0.5,
		                        m_uniqueQhPositions[i]-smallShift, true, 1);
	}
    //////////////////////////////////////////////////////////////////////////////////
    ...
    if(nbrQh==2)
    {
        ...
        for(int i=0; i<origNbr; ++i)
	    {
		    keepIndexes[i] = i;
	    }
        for(int i=0; i<origNbr; ++i)
        {
            for(int j=i+1; j<origNbr; ++j)
            {
        ...
        counter = 0;
	    for(int i=0; i<m_nbrW1W2; ++i)    //  Theta_2 objects
	    {
		    m_thetaFuncsW1W2[counter] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.5, 0.0,
		                                                           -m_uniqueW1W2[i]-smallShift, false, 1);
		    ++counter;
	    }
	    for(int i=0; i<m_nbrW1W2; ++i)    //  Theta_3 objects
	    {
		    m_thetaFuncsW1W2[counter] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.0, 0.0,
		                                                           -m_uniqueW1W2[i]-smallShift, false, 1);
		    ++counter;
	    }
	    for(int i=0; i<m_nbrW1W2; ++i)    //  Theta_4 objects
	    {
		    m_thetaFuncsW1W2[counter] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.0, 0.5,
		                                                           -m_uniqueW1W2[i]-smallShift, false, 1);
		    ++counter;
	    }
    }
    ...
    if(nbrQh==4)
    {
	    ...
	    for(int i=0; i<origNbr; ++i)
	    {
		    if(keepIndexes[i] != -1)
		    {
			    m_uniqueW1W2W3W4A[counter] = (listQh[i*nbrQh]+listQh[i*nbrQh+1]-listQh[i*nbrQh+2]
			                                -listQh[i*nbrQh+3])/2.0;
			    #if _TEST_MODE_
			    std::cout << "\t filtered (w1+w2-w3-w4)/2 " << counter << " is " 
			              << m_uniqueW1W2W3W4A[counter] << std::endl;
				#endif
			    ++counter;
		    }
	    }
        ...
        counter = 0;
	    for(int i=0; i<m_nbrW1W2W3W4A; ++i)    //  Theta_2 objects
	    {
		    m_thetaFuncsW1W2W3W4A[counter] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.5, 0.0,
		                                        -m_uniqueW1W2W3W4A[i]-smallShift, false, 1);
		    ++counter;
	    }
	    for(int i=0; i<m_nbrW1W2W3W4A; ++i)    //  Theta_3 objects
	    {
		    m_thetaFuncsW1W2W3W4A[counter] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.0, 0.0,
		                                        -m_uniqueW1W2W3W4A[i]-smallShift, false, 1);
		    ++counter;
	    }
	    for(int i=0; i<m_nbrW1W2W3W4A; ++i)    //  Theta_4 objects
	    {
		    m_thetaFuncsW1W2W3W4A[counter] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.0, 0.5,
		                                         -m_uniqueW1W2W3W4A[i]-smallShift, false, 1);
		    ++counter;
	    }
	    //  Case B
	    ...
	    for(int i=0; i<origNbr; ++i)
	    {
		    if(keepIndexes[i] != -1)
		    {
			    ...
			    #if _TEST_MODE_
			    std::cout << "\t filtered (w1-w2+w3-w4)/2 " << counter << " is " 
			              << m_uniqueW1W2W3W4B[counter] << std::endl;
				#endif
			    ++counter;
		    }
	    }
        ...
        counter = 0;
	    for(int i=0; i<m_nbrW1W2W3W4B; ++i)    //  Theta_2 objects
	    {
		    m_thetaFuncsW1W2W3W4B[counter] = new utilities::ThetaLookUp(m_wfData->Lx ,m_wfData->Ly, 0.5, 0.0,
		                                        -m_uniqueW1W2W3W4B[i]-smallShift, false, 1);
		    ++counter;
	    }
	    for(int i=0; i<m_nbrW1W2W3W4B; ++i)    //  Theta_3 objects
	    {
		    m_thetaFuncsW1W2W3W4B[counter] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.0, 0.0,
		                                        -m_uniqueW1W2W3W4B[i]-smallShift, false, 1);
		    ++counter;
	    }
	    for(int i=0; i<m_nbrW1W2W3W4B; ++i)    //  Theta_4 objects
	    {
		    m_thetaFuncsW1W2W3W4B[counter] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.0, 0.5,
		                                        -m_uniqueW1W2W3W4B[i]-smallShift, false, 1);
		    ++counter;
	    }
    }
	...
	for(int i=0; i<origNbr; ++i)
	{
		if(keepIndexes[i]!=-1)
		{
			...
			#if _TEST_MODE_
			std::cout << "\t filtered (w1+w2+...)/2.0 " << counter << " is " 
			          << m_uniqueCom[counter] << std::endl;
			#endif
			++counter;
		}
	}
	...
	counter = 0;
	for(int i=0; i<m_nbrCom; ++i)
	{
		m_thetaFuncsCom[counter] = new utilities::ThetaLookUp(m_wfData->Lx, m_wfData->Ly, 0.5, -0.5,
		                                -m_uniqueCom[i]+mooreReadComZero-smallShift, true, 1);
		++counter;
	}
	return;
}
	...
	const
{
    ...
    {
        std::cerr << "\nERROR: MooreRead::evaluate_wf(int n, dcmplx *z,int whichState)"
                  << " whichState argument must be between 0 and 2" << std::endl;
        exit(EXIT_FAILURE);
    }
	if(m_wfData->jastrowExponent>1)
	{
		std::cerr << "WARNING: MooreRead::evaluate_wf() is only currently programmed with"
		          <<" whichState nu=1 COM term" << std::endl;
		getchar();
	}
	dcmplx wfValue = dcmplx(0, 0);
	...
	for(int i=0; i<n; ++i)
	{
		...
		for(int j=i+1; j<n; ++j)
		{
			...
			{
				dcmplx tmpDcmplx=*(m_thetaFuncs[0]->thetaLookUpTable-diffx*Ly+diffy);
	...
	for(int i=0; i<n-1; ++i, p_row+=n)
	{
		...
		for(int j=i+1; j<n; ++j)
		{
			...
			#endif
			*(p_row+j) = *(m_thetaFuncs[whichState+1]->thetaLookUpTable+diffx*(2*Ly)+diffy) 
			           - *(m_thetaFuncs[0]->thetaLookUpTable+diffx*(2*Ly)+diffy);
			#if _TEST_MODE_ == 2
			...
			else if(diffx<0 && diffy>=0)
			{
				*(p_row+j) = 0;
				//	numerator theta_a
				dcmplx tmpDcmplx = *(m_thetaFuncs[whichState+1]->thetaLookUpTable-diffx*Ly+diffy);
			...
			*(p_row+j) = exp(*(p_row+j));
			...
			{
			    case 0:
			        std::cout << "\n\tNumerator: " << m_testValA << "\t" 
			                  << utilities::thetaFunction::GeneralisedJacobi(0.5, 0.0,
	                (dcmplx(real(latticeConfig[i]-latticeConfig[j]),imag(latticeConfig[i]-latticeConfig[j]))+smallShift)/(double)Lx,(double)Ly/Lx);
			        break;
			    case 1:
			        std::cout << "\n\tNumerator: " << m_testValA << "\t" 
			                  << utilities::thetaFunction::GeneralisedJacobi(0.0,0.0,
	                (dcmplx(real(latticeConfig[i]-latticeConfig[j]),imag(latticeConfig[i]-latticeConfig[j]))+smallShift)/(double)Lx,(double)Ly/Lx);
			        break;
			    case 2:
			        std::cout << "\n\tNumerator: " << m_testValA << "\t" 
			                  << utilities::thetaFunction::GeneralisedJacobi(0.0,0.5,
	                (dcmplx(real(latticeConfig[i]-latticeConfig[j]),imag(latticeConfig[i]-latticeConfig[j]))+smallShift)/(double)Lx,(double)Ly/Lx);
			        break;
			}        
			std::cout << "\n\tDenominator: " << m_testValB << "\t" 
			          << utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5,
	        (dcmplx(real(latticeConfig[i]-latticeConfig[j]),imag(latticeConfig[i]-latticeConfig[j]))+smallShift)/(double)Lx,(double)Ly/Lx)<<std::endl; 
			#endif
		}
	}
	...
	wfValue += utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5, 
	    (zcm-mooreReadComZero+smallShift)/(double)Lx,(double)Lx/Ly);
	#if _TEST_MODE_
	...
	const
{
    ...
    if(nbrQh==2)       //	Three wave function types are available in the Pfaffian term
    {
        if(whichState<0 || whichState>2)		
        {
            std::cerr << "ERROR: MooreRead::evaluate_wf(int n, dcmplx *z,int whichState)"
                      << " whichState argument must be between 0 and 2" << std::endl;
            exit(EXIT_FAILURE);
        }
    }
    if(nbrQh==4)       //	Four wave function types are available in the Pfaffian term
    {
        ...
        {
            std::cerr << "ERROR: MooreRead::evaluate_wf(int n, dcmplx *z,int whichState)"
                      << " whichState argument must be between 0 and 5" << std::endl;
            exit(EXIT_FAILURE);
        }
    }
	if(m_wfData->jastrowExponent>1)
	{
		std::cerr << "\nWARNING: MooreRead::evaluate_wf() is only currently programmed "
		          << "with whichState nu=1 COM term" << std::endl;
	}
	dcmplx wfValue = dcmplx(0, 0);
	//  Implement Jastrow factor part
	for(int i=0; i<n; ++i)
	...
	switch(nbrQh)
	{
	    case 2:
	    {
	        ...
	        if(m_uniqueQhPositions!=0)		
	        {
                ...
                #if _TEST_MODE_ == 2
                   std::cout << "\n\tLocation of quasi-hole(s) in list " << currW1 << " and " 
                             << currW2 << std::endl;
                #endif
		            ...
		            for(int j=i+1; j<n; ++j)
		            {
                        ...
                        {    
                            case 0:
                                ...
                                std::cout << "\n\t" << termIJ << "\t" << 
                                    utilities::thetaFunction::GeneralisedJacobi(0.5, 0.0,
                                    (dcmplx(real(latticeConfig[i]-latticeConfig[j]), imag(latticeConfig[i]-
                                    latticeConfig[j]))+(zQh[0]-zQh[1])/2.0+smallShift)/(double)Lx, (double)Ly/Lx)
                                         << std::endl;
                                std::cout << "argument of theta: " << 
                                    (dcmplx(real(latticeConfig[i]-latticeConfig[j]), imag(latticeConfig[i]-
                                    latticeConfig[j]))+(zQh[0]-zQh[1])/2.0+smallShift)/(double)Lx << std::endl;
                                std::cout << "\n\tCheck z" << j << "-z" << i << "+(w1-w2)/2 table value (should match)" << std::endl;
                                std::cout << "\n\t" << termJI << "\t" << 
                                    utilities::thetaFunction::GeneralisedJacobi(0.5, 0.0,
                                    (dcmplx(real(latticeConfig[j]-latticeConfig[i]),imag(latticeConfig[j]-
                                    latticeConfig[i]))+(zQh[0]-zQh[1])/2.0+smallShift)/(double)Lx, (double)Ly/Lx) << std::endl;
                                break;
                            case 1:
                                std::cout << "\n\tCheck z" << i << "-z" << j << "+(w1-w2)/2 table value (should match)" << std::endl;
                                std::cout << "\n\t" << termIJ << "\t" << 
                                    utilities::thetaFunction::GeneralisedJacobi(0.0, .0,
                                (dcmplx(real(latticeConfig[i]-latticeConfig[j]),imag(latticeConfig[i]-
                                latticeConfig[j]))+(zQh[0]-zQh[1])/2.0+smallShift)/(double)Lx, (double)Ly/Lx) << std::endl;
                                std::cout << "\n\tCheck z" << j << "-z" << i << "+(w1-w2)/2 table value (should match)" << std::endl;
                                std::cout << "\n\t" << termJI << "\t" 
                                          << utilities::thetaFunction::GeneralisedJacobi(0.0, 0.0,
                                    (dcmplx(real(latticeConfig[j]-latticeConfig[i]),imag(latticeConfig[j]-
	    ...
	    case 4:
	    {
	        ...
	        if(m_uniqueQhPositions!=0)		
	        {
                ...
                #if _TEST_MODE_ == 2
                   std::cout << "\n\tLocation of quasi-hole(s) in list " << currW1 << "," << currW2 
                             << "," << currW3 << ", and " << currW4 << std::endl;
                #endif
		        //  Determine also which value of (W1+W2-W3-W4)/2 we currently have
		        for(int k=0; k<m_nbrW1W2W3W4A; ++k)
		        {
		        ...
		        #endif
		        //  Determine also which value of (W1-W2+W3-W4)/2 we currently have 
		        for(int k=0; k<m_nbrW1W2W3W4B; ++k)
		        {
                        ...
                        {    
                            case 0:
                                ...
                                std::cout << "\n\t" << termIJ << "\t" 
                                          << utilities::thetaFunction::GeneralisedJacobi(0.5, 0.0,
                                (dcmplx(real(latticeConfig[i]-latticeConfig[j]), imag(latticeConfig[i]-
                                latticeConfig[j]))+(zQh[0]+zQh[1]-zQh[2]-zQh[3])/2.0+smallShift)/(double)Lx,(double)Ly/Lx) << std::endl;
                                std::cout << "\n\tCheck z" << j << "-z" << i << "+(w1+w2-w3-w4)/2 table value (should match)" << std::endl;
                                std::cout << "\n\t" << termJI << "\t" 
                                          << utilities::thetaFunction::GeneralisedJacobi(0.5, 0.0,
                                (dcmplx(real(latticeConfig[j]-latticeConfig[i]),imag(latticeConfig[j]-
                                latticeConfig[i]))+(zQh[0]+zQh[1]-zQh[2]-zQh[3])/2.0+smallShift)/(double)Lx,(double)Ly/Lx) << std::endl;
                                break;
                            case 1:
                                std::cout << "\n\tCheck z" << i << "-z" << j << "+(w1-w2+w3-w4)/2 table value (should match)" << std::endl;
                                std::cout << "\n\t" << termIJ << "\t" 
                            ...
                            case 5:
                                ...
                                std::cout << "\n\t" << termJI << "\t" 
                                          << utilities::thetaFunction::GeneralisedJacobi(0.0, 0.5,
                                (dcmplx(real(latticeConfig[j]-latticeConfig[i]),imag(latticeConfig[i]-
                                latticeConfig[j]))+(zQh[0]-zQh[1]+zQh[2]-zQh[3])/2.0+smallShift)/(double)Lx,(double)Ly/Lx) << std::endl;
                                break;
                        }
                        #endif
                ...
                for(int i=0; i<n-1; ++i, p_row+=n)
                {
	                for(int j=i+1; j<n; ++j)
	                {
                        ...
                        +utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5, (dcmplx(real(latticeConfig[i]), 
                            imag(latticeConfig[i]))-zQh[0]+smallShift)/(double)Lx, (double)Ly/Lx)
                        // theta_1 (z_i - w_2)
                        +utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5, (dcmplx(real(latticeConfig[i]), 
                            imag(latticeConfig[i]))-zQh[1]+smallShift)/(double)Lx, (double)Ly/Lx)
                        // theta_1 (z_j - w_3)  
                        +utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5,(dcmplx(real(latticeConfig[j]), 
                            imag(latticeConfig[j]))-zQh[2]+smallShift)/(double)Lx, (double)Ly/Lx)
                        // theta_1 (z_j - w_4)
                        +utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5,(dcmplx(real(latticeConfig[j]), 
                            imag(latticeConfig[j]))-zQh[3]+smallShift)/(double)Lx, (double)Ly/Lx)
                        )+
                        exp(termJI
                        // theta_1 (z_i - w_3)
                        +utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5,(dcmplx(real(latticeConfig[i]), 
                            imag(latticeConfig[i]))-zQh[2]+smallShift)/(double)Lx, (double)Ly/Lx)
                        // theta_1 (z_i - w_4)
                        +utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5,(dcmplx(real(latticeConfig[i]), 
                            imag(latticeConfig[i]))-zQh[3]+smallShift)/(double)Lx, (double)Ly/Lx)
                        // theta_1 (z_j - w_1)
                        +utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5,(dcmplx(real(latticeConfig[j]), 
                            imag(latticeConfig[j]))-zQh[0]+smallShift)/(double)Lx, (double)Ly/Lx)
                        // theta_1 (z_j - w_2)
                        +utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5,(dcmplx(real(latticeConfig[j]), 
                            imag(latticeConfig[j]))-zQh[1]+smallShift)/(double)Lx, (double)Ly/Lx)
                        );
                        *(p_row+j) = exp(log(tmpDcmplx) - utilities::thetaFunction::GeneralisedJacobi(0.5, -0.5,(dcmplx(real(diff), imag(diff))+smallShift)/(double)Lx, (double)Ly/Lx));
			        }
			    }  
	    ...
	    default:
	            std::cerr << "\nERROR: MooreRead::evaluate_wf() called with" << nbrQh 
	                      << "quasi-hole(s). The algorithm currently only handles 2 or 4 quasiholes." << std::endl;
	            exit(EXIT_FAILURE);
	        break;
	}
