...
//!
//! Specify the integrand function for overlap calculations, using
...
//!
//! Calculate the monopole harmonic function, which describes the single
    ...
    const
{
    ...
    for(int s=0; s<=max; ++s)
    {
        ...
        sum +=  sign*utilities::BinomialFromTable(l, s)*utilities::BinomialFromTable((int)(2*q+l), 
                (int)(q+l-m-s))*std::pow((1-x), (l-s))*std::pow((1+x), s);
    }
...
//!
//! Calculate the normalization factor of the monopole harmonic functions
//!
...
//!
//! Public interface to sequentially call all internal calculations
//!
...
{
    utilities::cout.MainOutput() << "\n\t=========================    CALCUALTING IQHE RSES  "
                                 << "  ========================="<<std::endl;
    int nbrParticles = (*optionList)["nbr-n"].as<int>();
    int nbrA = (*optionList)["nbr-a"].as<int>();
    if(nbrParticles>_MAX_NUMBER_)
    {
        std::cerr << "ERROR: IQHE spectrum not calculable for more than " << _MAX_NUMBER_ 
                  << " particles - the code needs to be modified." << std::endl;
        return;
    }
    ...
    for(int s=lzaMin; s<=lzaMax; ++s)
    {
        ...
        occupationBasis::AssignEnergies(occupationsList, energyLevels, spectrum, normalization, 
                                        nbrStates, nbrLevels);
        this->SpectrumToFile(optionList, spectrum, occupationsList.size(), s);
        delete[] spectrum;
    }
    delete[] energyLevels;
}
    ...
    const
{
    ...
    for(int m=0;m<nbrStates; ++m, zComponentAngularMomentum += 1.0)
    {
        ...
        for(int i=0; i<nbrLevels; ++i)
        {
            ...
            for(int j=0; j<nbrLevels; ++j, ++p_buffer)
            {
                integrandParams[3] = j;
                const double jNorm = this->MonopoleHarmonicNorm(q, j, zComponentAngularMomentum);
                integrand.params = integrandParams;
                gsl_integration_cquad(&integrand, lowerLimit, upperLimit, epsabs, epsrel, gsl_work, 
                                      &result, &error, &nevals);
...
//!
//! This function calls a LAPACK routine to diagonalize the overlap matrix
    ...
    const
{
    ...
    for(int i=0; i<nbrStates*nbrLevels; ++i, ++p_levels)
    {
        ...
        *(p_levels) = log(alpha/oneMinusAlpha);
        ...
        utilities::cout.DebuggingInfo() << "\tSingle particle pseudoenergy [" << i << "] = " 
                                        << *(p_levels) << std::endl;
    }
...
#if _DEBUG_
//!
//! A debugging function that can be used to check the angular momentum 
...
//!
//!	This function generates a file name string to identify output files
//!
...
//!
//! Assign the energy levels calculated by the function GenerateNewLevels()
    ...
    const
{
    ...
    f_out.close();
    utilities::cout.MainOutput() << "\n\t- SECTOR " << sector << " DATA WRITTEN TO FILE: "
