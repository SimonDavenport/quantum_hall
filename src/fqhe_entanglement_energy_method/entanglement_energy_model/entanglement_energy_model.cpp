...
void EntanglementEnergyModel::GenerateEntanglementEnergies(
    ...
    std::vector<double>& mp)        //!<    List of model parameters
    const
{
    ...
    for(int m=0; m<nbrStates; ++m, z+=1.0)
    {
        for(int l=0; l<nbrLevels; ++l, ++p_energy)
        {
            ...
            *(p_energy) = mp[0] + (mp[1] + (mp[2] + (mp[3] + mp[4]*z*z)*z)*z)*z +
    ...
    const
{
	...
	fileName << (*optionList)["out-path"].as<std::string>() << "/eigenvalues_entanglement_energy_model"
	         << "_n_" << (*optionList)["nbr-n"].as<int>() << "_na_" << (*optionList)["nbr-a"].as<int>()
    ...
    const
{
    ...
    {
        std::cerr << "ERROR: entanglement energy model not calculable for more than " 
                  << _MAX_NUMBER_  << " particles - the code needs to be modified." << std::endl;
        return;
    }
    ...
    utilities::cout.MainOutput() << "\n\t\tMONOPOLE STRENGTH (=FLUX/2)\t\t" 
                                 << monopoleStrength << std::endl;
    ...
    this->GenerateEntanglementEnergies(energyLevels, monopoleStrength, normalization, 
                                       nbrLevels, nbrStates, mp);
    this->GenerateEntanglementEnergies(qLevels, monopoleStrength, qNormalization, 
                                       nbrLevels, nbrStates, qModelParameters);
    utilities::cout.AdditionalInfo() << "\tnormalization = " << normalization 
                                     << std::endl << std::endl;
    for(int i=0; i<nbrStates*nbrLevels; ++i)
    {
        utilities::cout.SecondaryOutput() << "\tSingle particle energy level [" << i 
                                          << "] = " << energyLevels[i] << std::endl;
    }
    for(int i=0; i<nbrStates*nbrLevels; ++i)
    {
        utilities::cout.SecondaryOutput() << "\tSingle particle q level [" << i << "] = " 
                                          << qLevels[i] << std::endl;
    }
    ...
    for(int s=lzaMin; s<=lzaMax; ++s)
    {
        ...
        occupationBasis::AssignEnergies(occupationsList, energyLevels, spectrum, normalization, 
                                        nbrStates, nbrLevels);
        occupationBasis::AssignSquaredEnergies(occupationsList, qLevels, spectrum, qNormalization,
                                               nbrStates, nbrLevels);
        for(unsigned int i=0; i<occupationsList.size(); ++i)
        {
        ...
        if((*optionList)["use-perturbation"].as<bool>())
        {
            ...
            utilities::QuickSort<double, int, _ASCENDING_ORDER_>(spectrum, spectrumLabels.data(), 
                                                                 spectrumLabels.size());
            utilities::QuickSort<double, std::vector<std::bitset<_MAX_NUMBER_>>, _ASCENDING_ORDER_>(
                spectrumCopy, occupationsList.data(), occupationsList.size());
            delete[] spectrumCopy;
