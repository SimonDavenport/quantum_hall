	...
	m_pfaff(0)
{
	...
	if(m_nLastCluster)  
	{
	    nClusterList.push_back(m_nLastCluster);
	}
	...
	std::cout << "\tTotal number of cluster permutations = " << m_clusterPermutations.size() 
	          << std::endl << std::endl;
}

//!
//!	Recursively generate unique subsets of the particle indices in 
//! different clusters
//!
