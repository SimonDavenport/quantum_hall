...
//!
//!	Sets parameter values using command line arguments
//!
	...
	m_cfData(compositeFermionData)
{
	...
	{
		std::cerr << "\n\tWARNING: p<=0. Must have at least 1 flux attached. Setting fluxAttatched=1." 
		          << std::endl << std::endl;
	...
	{
		std::cerr << "\n\tERROR: For negative effective field, no coded algorithm for flux attatched >=2." 
		          << std::endl << std::endl;
		exit(EXIT_FAILURE);
	}
	...
	{
		std::cerr << "\n\tERROR: Cannot work with partially occupied spinful CF wave functions." 
		          << std::endl << std::endl;
		exit(EXIT_FAILURE);
	}
	if(!m_cfData->NEF && m_cfData->LLdown>1 &&  m_cfData->LLup!=m_cfData->LLdown)
	{
		std::cerr << "\n\tERROR: Cannot work with such cases yet - a bug remains in "
		          << "NonPolarisedPEF algorithm in this case." << std::endl << std::endl;
		exit(EXIT_FAILURE);
	}
	...
	m_wfData->shift = m_wfData->nbr*(double)m_wfData->fillDenominator/m_wfData->fillNumerator-
	                  (2*m_preData->effectiveMonopole+2*m_wfData->jastrowExponent*(m_wfData->nbr-1));
	...
	// Take |Q| for use in subsequent calculations
	m_preData->effectiveMonopole = fabs(m_preData->effectiveMonopole);  
	LLup = abs(LLup);
	LLdown = abs(LLdown);
    ...
    {
	    double m = -m_preData->effectiveMonopole;
	    int degen = (int)(2.0*m_preData->effectiveMonopole)+1;
	    int totdegen = degen;
	    int ll = 0; 
	    m_preData->llOccupationNumbers[ll] = degen;
	    for(int d=0; d<degen; ++d, ++m)
	    {
		    m_preData->cfLz[d] = m;
	    ...
	    while(ll+1<LLup)
	    {
		    ...
		    m_preData->llOccupationNumbers[ll] = degen;
		    for(int d=totdegen; d<(totdegen+degen); ++d, ++m)
		    {
	    ...
	    //	Otherwise quit
	    for(int l=0; l<LLup; ++ l)
	    {
		    if(m_preData->llOccupationNumbers[l]==0)
		    {
	...
	//////      Generate output file name       ////////////////////////////////////
	{
	    ...
	    if(m_cfData->bsMode)
	    {
		    if(m_cfData->NEF)
		    {
			    name << "Pfaffian*2CF_(-" << LLup << ",-" << LLdown;
			    name << ") Bondersen--Slingerland (negative effective field)";
		    }
		    else
		    {
			    name << "Pfaffian*2CF_(" << LLup << "," << LLdown;
			    name << ") Bondersen--Slingerland (positive effective field)";
		    }
	    }
...
//////	FOR INTERNAL OPERATIONS    
	...
	m_preData->maxBinom = (int)(2*m_preData->effectiveMonopole+std::max(abs(m_cfData->LLup),
	                                                                    abs(m_cfData->LLdown)))+1;
	...
	for(int i=0; i<m_preData->maxBinom; ++i)
	{
	   for(int j=0; j<m_preData->maxBinom; ++j)
	   {
		   ...
		   utilities::cout.DebuggingInfo() << "\t" << i << " C " << j << " = " 
		                                   << m_preData->binomList[i*m_preData->maxBinom+j] << std::endl;
	   }
	}
	...
	if(m_cfData->NEF)
	{
		...
		this->GeneratePreFactorNumber(m_wfData->nbr,std::max(abs(m_cfData->LLup), abs(m_cfData->LLdown)), 
		                              m_preData->nbrEachLL);
	}
	////////////////////////////////////////////////////////////////////////////////
	...
	m_lowPrec = new ArbitraryPrecisionCfWavefunctions<dcmplx, double, 64>(
	                wavefunctionData, compositeFermionData, m_preData);
	//	high precision using the mpf type form the gmp library
	m_precLevel1 = new ArbitraryPrecisionCfWavefunctions<mpfCmplx, mpf_t, _PREC_LEVEL1_>(
	                    wavefunctionData, compositeFermionData, m_preData);
	m_precLevel2 = new ArbitraryPrecisionCfWavefunctions<mpfCmplx, mpf_t, _PREC_LEVEL2_>(
	                    wavefunctionData, compositeFermionData, m_preData);
	m_precLevel3 = new ArbitraryPrecisionCfWavefunctions<mpfCmplx, mpf_t, _PREC_LEVEL3_>(
	                    wavefunctionData, compositeFermionData, m_preData);
	m_precLevel4 = new ArbitraryPrecisionCfWavefunctions<mpfCmplx, mpf_t, _PREC_LEVEL4_>(
	                    wavefunctionData, compositeFermionData, m_preData);
	//	NOTE the maximum precision is _PREC_LEVEL4_ (see header file for definitions)
	#endif
	////////////////////////////////////////////////////////////////////////////////
}
...
{	
	...
	if(m_preData->llOccupationNumbers!=0)	
	{
	    delete[] m_preData->llOccupationNumbers;
	}
	utilities::cout.DebuggingInfo() << "delete m_preData->cfLz" << std::endl;
	if(m_preData->cfLz!=0)					
	{
	    delete[] m_preData->cfLz;
	}
	utilities::cout.DebuggingInfo() << "did dtor" << std::endl;
	if(m_preData->nbrEachLL!=0)				
	{
	    delete[] m_preData->nbrEachLL;
	}
	delete m_preData;
}
	...
	const
{
	...
	for (int j=0; j<n; ++j){
	utilities::cout.DebuggingInfo() << real(*(u+j)) << "+" << imag(*(u+j)) << "I," << std::endl;}
	utilities::cout.DebuggingInfo() << "Here are the v co-ordinates:" << std::endl;
	for (int j=0; j<n; ++j){
	utilities::cout.DebuggingInfo() << real(*(v+j)) << "+" << imag(*(v+j)) << "I," << std::endl;}
	...
	if(m_cfData->NEF)       //	NEGATIVE EFFECTIVE FIELD CF WAVE FUNCTIONS
	{
		...
		if(m_cfData->spinPolarised)     //	SPIN POLARIZED
		{
			...
			if(conditionNumber>FQHE::condTolL2 || std::isinf(real(testVal)) || std::isnan(real(testVal)))
			{
			    ...
			    ////////////////////////////////////////////////////////////////////////////////
			    #if _DEBUG_
			    utilities::cout.DebuggingInfo() << "\n\tdcmplx value value: " << testVal 
			                                    << "\tmpf_t Level1-precision value: " << testVal1 << std::endl;
			    #endif
			    ////////////////////////////////////////////////////////////////////////////////
				...
				if(this->IncreasePrecCondition(testVal, testVal1, maxPrecLevel))
				{
					std::cerr << "\n\tERROR: MAXIMUM PRECISION REACHED WITHOUT CONVERGENCE."
					          << "SINGULAR WAVE FUNCTION OR OVERFLOW ERROR. " << std::endl;
		...
		else if(m_cfData->altProjection)		//	SPIN NON-POLARIZED SEPARATE UP AND DOWN PROJECTION
		{
			...
			testVal += m_lowPrec->PolarisedNEF(m_wfData->nbrDown, m_cfData->LLdown, 
			                                   u+m_wfData->nbrUp, v+m_wfData->nbrUp);
            if(std::isinf(real(testVal)) || std::isnan(real(testVal)))
            {
                std::cerr << "RECORDED OVERFLOW ERROR" << std::endl;
            }
			if(conditionNumber>FQHE::condTolL2 || std::isinf(real(testVal)) || std::isnan(real(testVal)))
			{
			    ...
			    testVal1 += m_precLevel1->PolarisedNEF(m_wfData->nbrDown, m_cfData->LLdown, 
			                                           u+m_wfData->nbrUp, v+m_wfData->nbrUp);
			    ////////////////////////////////////////////////////////////////////////////////
			    #if _DEBUG_
			    utilities::cout.DebuggingInfo() << "\n\tdcmplx value value: " << testVal 
				...
				if(this->IncreasePrecCondition(testVal, testVal1, maxPrecLevel))
				{
					...
					testVal1 += m_precLevel2->PolarisedNEF(m_wfData->nbrDown, m_cfData->LLdown,
					                                       u+m_wfData->nbrUp, v+m_wfData->nbrUp);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level2-precision value: " 
				...
				if(this->IncreasePrecCondition(testVal, testVal1, maxPrecLevel))
				{
					...
					testVal1 += m_precLevel3->PolarisedNEF(m_wfData->nbrDown, m_cfData->LLdown, 
					                                       u+m_wfData->nbrUp,v+m_wfData->nbrUp);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level3-precision value: " 
				...
				if(this->IncreasePrecCondition(testVal, testVal1, maxPrecLevel))
				{
					...
					testVal1 += m_precLevel4->PolarisedNEF(m_wfData->nbrDown, m_cfData->LLdown,
					                                       u+m_wfData->nbrUp, v+m_wfData->nbrUp);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level4-precision value: "
		...
		{
			testVal = m_lowPrec->NonPolarisedNEF(m_wfData->nbrUp, m_wfData->nbrDown, m_cfData->LLup, 
			                                     m_cfData->LLdown, u, v);
            if(std::isinf(real(testVal)) || std::isnan(real(testVal)))
            {
                std::cerr << "RECORDED OVERFLOW ERROR" << std::endl;
			...
			{
			    ++maxPrecLevel;
                testVal1 = m_precLevel1->NonPolarisedNEF(m_wfData->nbrUp, m_wfData->nbrDown, 
                                                         m_cfData->LLup, m_cfData->LLdown, u, v);
                ////////////////////////////////////////////////////////////////////////////////
			    #if _DEBUG_
			    utilities::cout.DebuggingInfo() << "\n\tdcmplx value value: " << testVal 
			                                    << "\tmpf_t Level1-precision value: " << testVal1 << std::endl;
				...
				{
					testVal = testVal1;
					testVal1 = m_precLevel2->NonPolarisedNEF(m_wfData->nbrUp, m_wfData->nbrDown,
					                                         m_cfData->LLup, m_cfData->LLdown, u, v);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level2-precision value: " 
				...
				{
					testVal = testVal1;
					testVal1 = m_precLevel3->NonPolarisedNEF(m_wfData->nbrUp, m_wfData->nbrDown,
					                                         m_cfData->LLup, m_cfData->LLdown, u, v);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level3-precision value: " 
				...
				{
					testVal = testVal1;
					testVal1 = m_precLevel4->NonPolarisedNEF(m_wfData->nbrUp, m_wfData->nbrDown,
					                                         m_cfData->LLup, m_cfData->LLdown, u, v);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level4-precision value: " 
	...
	else								//	POSITIVE EFFECTIVE FIELD CF WAVE FUNCTIONS
	{
		...
		else if(m_cfData->altProjection)		//	SPIN NON-POLARIZED SEPARATE UP AND DOWN PROJECTION
		{
			if(n>FQHE::maxSwitch)			//	above a certain number, use high precision
			{
				...
				testVal += m_lowPrec->PolarisedPEF(m_wfData->nbrDown, m_cfData->LLdown, 
				                                   u+m_wfData->nbrUp, v+m_wfData->nbrUp);
				testVal1 = m_precLevel1->PolarisedPEF(m_wfData->nbrUp, m_cfData->LLup, u, v);
				testVal1 += m_precLevel1->PolarisedPEF(m_wfData->nbrDown, m_cfData->LLdown, 
				                                       u+m_wfData->nbrUp, v+m_wfData->nbrUp);
                ++maxPrecLevel;
				////////////////////////////////////////////////////////////////////////////////
				#if _DEBUG_
				...
				if(this->IncreasePrecCondition(testVal, testVal1, maxPrecLevel))
				{
					...
					testVal1 += m_precLevel2->PolarisedPEF(m_wfData->nbrDown, m_cfData->LLdown,
					                                       u+m_wfData->nbrUp, v+m_wfData->nbrUp);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level2-precision value: " 
				...
				if(this->IncreasePrecCondition(testVal, testVal1, maxPrecLevel))
				{
					...
					testVal1 += m_precLevel3->PolarisedPEF(m_wfData->nbrDown, m_cfData->LLdown,
					                                       u+m_wfData->nbrUp, v+m_wfData->nbrUp);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level3-precision value: " 
				...
				if(this->IncreasePrecCondition(testVal, testVal1, maxPrecLevel))
				{
					...
					testVal1 += m_precLevel4->PolarisedPEF(m_wfData->nbrDown, m_cfData->LLdown,
					                                       u+m_wfData->nbrUp, v+m_wfData->nbrUp);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level4-precision value: " 
			...
			else
			{
				...
				wfValue += m_lowPrec->PolarisedPEF(m_wfData->nbrDown, m_cfData->LLdown, 
				                                   u+m_wfData->nbrUp, v+m_wfData->nbrUp);
			}
		}
		else    //	SPIN NON-POLARIZED COMBINED UP-DOWN PROJECTION
		{
			if(n>FQHE::maxSwitch)			//	Above a certain number, use high precision
			{
				testVal = m_lowPrec->NonPolarisedPEF(m_wfData->nbrUp, m_wfData->nbrDown, 
				                                     m_cfData->LLup, m_cfData->LLdown, u, v);
				testVal1 = m_precLevel1->NonPolarisedPEF(m_wfData->nbrUp, m_wfData->nbrDown,
				                                         m_cfData->LLup, m_cfData->LLdown, u, v);
                ++maxPrecLevel;
				////////////////////////////////////////////////////////////////////////////////
				#if _DEBUG_
				...
				{
					testVal = testVal1;
					testVal1 = m_precLevel2->NonPolarisedPEF(m_wfData->nbrUp, m_wfData->nbrDown,
					                                         m_cfData->LLup, m_cfData->LLdown, u, v);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level2-precision value: " 
				...
				{
					testVal = testVal1;
					testVal1 = m_precLevel3->NonPolarisedPEF(m_wfData->nbrUp, m_wfData->nbrDown,
					                                         m_cfData->LLup, m_cfData->LLdown, u, v);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level3-precision value: "
				...
				{
					testVal = testVal1;
					testVal1 = m_precLevel4->NonPolarisedPEF(m_wfData->nbrUp, m_wfData->nbrDown,
					                                         m_cfData->LLup, m_cfData->LLdown, u, v);
					////////////////////////////////////////////////////////////////////////////////
					#if _DEBUG_
					utilities::cout.DebuggingInfo() << "\n\tmpf_t Level4-precision value: " 
			...
			{
				wfValue = m_lowPrec->NonPolarisedPEF(m_wfData->nbrUp, m_wfData->nbrDown,
				                                     m_cfData->LLup, m_cfData->LLdown, u, v);
			}
		}
	}
    ...
    const double m)   //!<    z-component of angular momentum: from -q-l to q+l   
{
    ...
    if((q+l-m)>(q+l+m))
    {
        norm *= sqrt(utilities::DivFactorial<double>((q+l-m), (2.0*q+l))
                *utilities::DivFactorial<double>((q+l+m), l));
    }
    else
    {
        norm *= sqrt(utilities::DivFactorial<double>((q+l-m), l)
                *utilities::DivFactorial<double>((q+l+m), (2.0*q+l)));
    }
    return norm;
}
#endif
