...
class EntanglementEnergyModel
{
    ...
    void GenerateEntanglementEnergies(double* energyLevels,const double q,double& normalization,const int nbrLevels,const int nbrStates,std::vector<double>& modelParameters) const;

    void ParametersFromFile(boost::program_options::variables_map* optionList,std::vector<double>& modelParameters,std::vector<double>& qModelParameters) const;
    ...
    void ApplyPerturbation(std::vector<std::vector<std::bitset<_MAX_NUMBER_> > >& occupationsList,
    double* energyLevels,//double* qLevels,
    double* spectrum,const int nbrStates,std::vector<double>& modelParameters) const;

    public:

    void BuildEntanglementEnergyModel(boost::program_options::variables_map* optionList,
                                      const int lzaMin,const int lzaMax) const;
