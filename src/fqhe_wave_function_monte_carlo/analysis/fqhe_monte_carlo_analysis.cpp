...
////////////////////////////////////////////////////////////////////////////////
//! \brief Default constructor for analysis options - sets default values
//!
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
//! \brief Initialize the analysis options struct variables from command line
//! arguments
//!
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
//! \brief Constructor from analysis options
//!
////////////////////////////////////////////////////////////////////////////////
...
AnalysisMethods::~AnalysisMethods()
{
	//std::cout<<"called this dtor"<<std::endl;
...
////////////////////////////////////////////////////////////////////////////////
//! \brief Initialize analysis methods options from command line arguments
//!
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
//! \brief Print the file name and generate a text file containing 
//! that information
//!
////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief A function to deal with generating file names and opening files
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief Attempt to open all relavant configuration data files and count the
...
int AnalysisMethods::LookForConfigurationDataFiles()
{
	...
	//	Exit the program if no data files are found
	
	if(exitFlag==EXIT_FAILURE) return EXIT_FAILURE;
	
	//	Now determine the number of data files available
	while(exitFlag==0)
	{
		//	Determine number of data samples in each file
		if(wfData.geometry==FQHE::_TORUS_)
		{
			f_config.seekg(0,std::ios::end);
			size += f_config.tellg();
			//	Count that you skip the first 'skip' samples 
	...
	//	check that all the files are now closed
	if(f_theta.is_open()!=0)	f_theta.close();
	if(f_phi.is_open()!=0)		f_phi.close();
	if(f_config.is_open()!=0)	f_config.close();
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief	Read in a single configuration from a file (sphere geometry)
//!
//////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief	Read in a single configuration from a file (disc geometry)
//!
//////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief Read in a single configuration from a file (torus geometry)
//!
//////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief Convert theta,phi co-ordinates to u,v co-ordinates
//!
//////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief Check that particles did not get stuck at the north pole
//!
//////////////////////////////////////////////////////////////////////////////////
    ...
    const int k)      //!<    MC sample index
{
	...
	for(int i=0;i<wfData.nbr;++i)
	{
		...
		if(i==flag2 && cos(theta[i])<=0.999999 && flag!=0)
		{
			...
			flag--;
			if(flag==0)std::cout<<"\tOK: Particle is now unstuck!"<<std::endl;
		}
	}
}
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief This function will resample the energy value if requested, 
    ...
    double &meanEnergy)     //!<    Address of mean energy value to be returned
{
	...
	for(int j=0;j<nbrResamples;++j)
	{
		...
		//std::cout<<"duplicatedIndices.size() = "<<duplicatedIndices.size()<<std::endl;
	    
		runningResample=0.0;
		//for(int k=0;k<nbrSamples;++k)
		//{
		//	runningResample+=*(energyStore+mt.genrand_int31() % nbrSamples)/nbrSamples;
		//}
...
//////////////////////////////////////////////////////////////////////////////////
//!	\brief A function to deal with the relevant background data, based on the 
...
void AnalysisMethods::InitializeMethods()
{
	...
	if(options.partialCoulomb)
	{
		...
		{
			std::cout<<"Effective potential (sphereRadius)^("<<options.partialCoulombType-6<<")*exp(-sphereRadius) (up-up pairs only)";
		}
		else if(options.partialCoulombType>13 && options.partialCoulombType<=20)
		{
			std::cout<<"Effective potential (sphereRadius)^("<<options.partialCoulombType-13<<")*exp(-sphereRadius) (up-down pairs only)";
		}
	}
	...
	if(options.pairCorrel)
	{
		...
		{
			std::cout<<"\tCannot open file: "<<fileName.str()<<std::endl<<std::endl;
			exit(EXIT_FAILURE);
		}
	...
	if(options.dens)
	{
		...
		//	initialize density calculation
	...
	if(options.coulomb)
	{
		...
		std::cout<<"\n\tAllocated "<<(double)nbrSamples*sizeof(double)/(1024*1024)<<" mb to store coulomb energy values"<<std::endl<<std::endl;
	...
	if(options.secllCoulomb)
	{
		//	initialize second LL Coulomb type interaction
		
		//  TODO compute 2nd LL background integral from scratch
		
		secondEnergy=new double[nbrSamples];
		
		std::cout<<"\n\tAllocated "<<(double)nbrSamples*sizeof(double)/(1024*1024)<<" mb to store second LL coulomb energy values"<<std::endl<<std::endl;
		
		//	Read in the background data files
		fileName.str("");
		...
		{
			std::cout<<"\tERROR: cannot open background data file:"<<fileName.str()<<std::endl;
			exit (EXIT_FAILURE);
		}
		...
		for(int j=0;j<(int)wfData.flux;++j) 
		{
			...
			//std::cout<<this->secondBg<<std::endl;
		}
		...
		else if(options.secllType=="sphere")
		{
			...
			{
				std::cout<<"\tERROR: cannot open file:"<<fileName.str()<<std::endl;
				exit(EXIT_FAILURE);
			}
		...
		{
			std::cout<<"\tERROR: unknown second LL type: "<<options.secllType<<std::endl;
			exit(EXIT_FAILURE);
		}
	...
	if(options.finiteThickness)
	{
		...
		//std::cout<<"\tAllocated "<<THICKNESS_POTENTIAL_FILE_SIZE*sizeof(double)/(1024*1024)<<"mb to store finite thickness potential"<<std::endl;
		fileName.str("");
		...
		if (f_background.is_open()==0)
		{
			std::cout<<"\tERROR: cannot open finite thickness background energy file: "<<fileName.str()<<std::endl;
			exit(EXIT_FAILURE);
		}
		...
		{
			std::cout<<"\tERROR: cannot open finite thickness potential file: "<<fileName.str()<<std::endl;
			exit(EXIT_FAILURE);
		}
		...
		{
			std::cout<<"\tERROR: options.minThickness "<<options.minThickness<<" out of range."<<std::endl;
			exit(EXIT_FAILURE);
		}
		
		if(options.maxThickness>10)
		{
			std::cout<<"\tERROR: options.maxThickness "<<options.maxThickness<<" out of range\n";
			std::cout<<"\t(insufficient backgorund data provided)"<<std::endl;
			exit(EXIT_FAILURE);
		}
		
		if((int)(options.thicknessStepSize*THICKNESS_BG_SIZE)%10!=0)
		{
			std::cout<<"\tERROR: thickness parameter step size "<<options.thicknessStepSize<<" not possible\n";
			std::cout<<"\t(insufficient backgorund data provided)"<<std::endl;
			exit(EXIT_FAILURE);
		}
	...
	if(options.partialCoulomb)
	{
		...
		std::cout<<"\tAllocated "<<(double)nbrSamples*sizeof(double)/(1024*1024)<<" mb to store partial coulomb energy values"<<std::endl;
		...
		if(options.partialCoulombType>2)
		{
			...
			for(int j=0;j<(int)wfData.flux;++j) 
			{
				...
				//std::cout<<this->partialBg<<std::endl;
			}
			//	Background values are storred without the factor of n^2: we need to put this back in!
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief	Calculate coulomb energy in the sphere geometry using a chord distance 
//! measure
//!
//////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief This function calculates the average energy from a bilayer type 
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief This function calculates the energy due to a charge plate at distance 
...
//////////////////////////////////////////////////////////////////////////////////
//! brief This function evaluates the chord distances for all pairs of particles
    ...
    dcmplx *v)  //!<    Pointer to v data
{
	...
	for(int i=0;i<nup;++i)
	{
		for(int j=i+1;j<nup;++j)
		{
			//Calculate the arc distances between all pairs of points:
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief This function evaluates chord distance to the north pole for all 
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief	This function evaluates the electrocstatic energy due to a
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief This function evaluates the electrostatic energy due to a modified LLL
    ...
    dcmplx *v)  //!<    Pointer to v data
{
	...
	if(bg<=0 || bg>1000)
	{
		std::cout<<"ERROR: Finite thckness parameter beyond range of stored background values."<<std::endl;
		exit(EXIT_FAILURE);
	}
    ...
    dcmplx *v)          //!<    Pointer to v data
{
	...
	//std::cout<<"BGvalue: "<<energy<<std::endl;
	//getchar();
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief This function calculates only the mean and (estimated) 
...
//////////////////////////////////////////////////////////////////////////////////
//! \brief  This function tallys up all of the energy values, determines 
...
void AnalysisMethods::FinaliseMethods()
{
	if(options.pairCorrel)
	{
		...
		if(wfData.nbrDown>0)
		{
			...
			//	up-down is normalised to the value min(nup,ndown)/2*wfData.sphereRadius
			{
			    ...
			    //std::cout<<"\n\tNormalised pair correlation"<<std::endl;
	...
	if(options.coulomb)
	{
		...
		std::cout<<"\tMean Coulomb energy per electron:\n\n\t"<<coulombMeanEnergy<<" +- "<<coulombStdev<<std::endl<<std::endl;
		std::cout<<"\tMean Coulomb energy per electron [included 1/sqrt(n/(n-1)) factor]\n\n\t";
		std::cout<<coulombMeanEnergy*sqrt((double)(wfData.nbr-1.0)/(wfData.nbr))<<" +- "<<coulombStdev*sqrt((double)(wfData.nbr-1.0)/wfData.nbr)<<std::endl<<std::endl;
	}
	...
	if(options.secllCoulomb)
	{
		...
		std::cout<<" energy per electron:\n\n\t"<<secondMeanEnergy<<" +- "<<secondStdev<<std::endl<<std::endl;
	}
	...
	if(options.partialCoulomb)
	{
		...
		if(options.partialCoulombType>6 && options.partialCoulombType<=13)
		{
			std::cout<<"\tMean Effective potential (r)^("<<options.partialCoulombType-6<<")*e^-r (up-up pairs only)"<<std::endl;
		}
			
		if(options.partialCoulombType>13 && options.partialCoulombType<=20)	
		{
			std::cout<<"\tMean Effective potential (r)^("<<options.partialCoulombType-13<<")*e^-r (up-down pairs only)"<<std::endl;
		}
