...
//	Wrapper class to contain high precision functions and variables
#include <utilities/wrappers/high_precision_wrapper.hpp>
//	For linear algebra functions (for the Slater determinant)
#include <utilities/mathematics/dense_linear_algebra.hpp>
//	For combinatoric functions (for binomial coefficients/factorials)
...
////////////////////////////////////////////////////////////////////////////////
//!	\brief	A data structure to contain a basic set of composite fermion
//! wave function data
//!
////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////
//!	\brief	Contains all data that can only needs to be calculated once	
...
class ArbitraryPrecisionCfWavefunctions
{
	...
	//	Declare template links to high precision wrapper class 
	//	(see highPrecisionWrapper.h)
	...
	////////////////////////////////////////////////////////////////////////////////
		...
		//	Keep a record of the number of calls to the routines here
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//	
	...
	m_pArray(0)
	{
		...
		//	Allocate memory to store arbitrary precision arrays
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
	...
	{	
		////////////////////////////////////////////////////////////////////////////
		#if _CF_BENCHMARK_MODE_
			utilities::cout.SecondaryOutput()<<"Number of calls to this precision: "<<m_noCalls<<std::endl;
		#endif
		////////////////////////////////////////////////////////////////////////////

		//	Delete only if defined
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
	...
	//	Declare class internal functions
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
	...
	{       
        //////      Calculate the list of pre-factors       ////////////////////////
        
        {
		    utilities::HpWrap<V,P> realTmp;     //  Arbitrary precision temporary value
		    utilities::HpWrap<V,P> *p_list = facList;
		                                    //  pointer to a memory address in the factor list
		    ...
		    for(int d=0;d<m_preData->llOccupationNumbers[0];d++)
		    {
                ...
                double norm = FQHE::MonopoleHarmonicNorm(m_preData->effectiveMonopole,0,m_preData->cfLz[d])*std::pow(FQHE::extraNormFactor,n/2);
            ...
            for(int ll=1,degen=0,cumulativeDegen=m_preData->llOccupationNumbers[0];ll<nbrLLs;ll++,cumulativeDegen+=degen)
		    {
			    ...
			    for(int d=cumulativeDegen;d<cumulativeDegen+degen;d++)
			    {
                    ...
                    double norm = FQHE::MonopoleHarmonicNorm(m_preData->effectiveMonopole,ll,m_preData->cfLz[d])*std::pow(FQHE::extraNormFactor,n/2);
						    ...
						    for(int t=(int)(m_preData->effectiveMonopole+m+s);t<=(int)(n-1-m_preData->effectiveMonopole+m-ll+s);t++)
						    {
							    if(t>=0 && t<n)
							    {
								    ...
								    *p_list=realTmp;
								    ...
								    (*p_list)*=realTmp;
								    (*p_list).MulBy(m_preData->binomList[(int)((2*m_preData->effectiveMonopole+ll)*m_preData->maxBinom+m_preData->effectiveMonopole+m+s)]*m_preData->binomList[ll*m_preData->maxBinom+s],(*p_list));
								    (*p_list).MulBy(1.0/norm,(*p_list));
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
		...
		utilities::HpWrap<U,P> *uProd)		    //!<	Pointer to a table to store values of u, u*u etc. 
	{
		...
		for(int j=0;j<n;j++)
		{
			for(int k=1;k<n;k++)
			{
				utilities::cout.DebuggingInfo()<<"\t["<<j<<"]["<<k<<"]="<<symmPolyArray[j*n+k].Get()<<std::endl;
			}
		}
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
    ...
    {
		utilities::HpWrap<U,P>	tempU[n];	//	Temporary wrapper value for the u co-ordinates
        utilities::HpWrap<U,P>	tempV[n];	//	Temporary wrapper value for the v co-ordinates
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
	...
	{
		//	Declare local variables
		utilities::HpWrap<U,P> tmp1,tmp2,tmp3;  //		Temp variables
		utilities::HpWrap<U,P> runTot,runTot2;  //		Dummy variables to accumulate loop values
		utilities::HpWrap<U,P> *p_fArray;		//		Pointer to m_fArray
		utilities::HpWrap<U,P> *p_pArray;		//		Pointer to point to pArray

		//	To save memory access time, it is convenient to set the ratios of v[j]/u[j]
		//	to be the variable z[j].
		...
		for(int a=0;a<nbrLLs;a++){
			for(int b=0;b<nbrLLs;b++){
				...
				{
					for(int j=0;j<n;j++){
						utilities::cout.DebuggingInfo()<<"m_fArray["<<a<<"]["<<b<<"]["<<j<<"]="<<(*(m_fArray+(a*nbrLLs+b)*n+j)).Get()<<std::endl;
					}}}}getchar();

		#endif
		////////////////////////////////////////////////////////////////////////////////

		//	Second, determine P_j(alpha, beta) functions. These are given e.g in J.K. Jain "Composite Fermions" appendix J.2.2. pp. 496--497
		...
		if(nbrLLs>2)
		{
			...
			//	Implement P_j(1,1)=m_cfData->halfFluxAttatched^2*f_j(0,1)*f_j(1,0)-m_cfData->halfFluxAttatched*f(1,1);
		...
		if(nbrLLs>3)
		{
			...
			//	Implement P_j(0,3)=m_cfData->halfFluxAttatched*f_j(0,1)*[m_cfData->halfFluxAttatched^2*f_j(0,1)^2-3*m_cfData->halfFluxAttatched*f_j(0,2)]+2*m_cfData->halfFluxAttatched*f_j(0,3);
			p_pArray=pArray+3*n;
			p_fArray=m_fArray+1*n;
			...
			//	Implement P_j(2,1)=m_cfData->halfFluxAttatched*f_j(0,1)[m_cfData->halfFluxAttatched^2*f_j(1,0)^2-m_cfData->halfFluxAttatched*f_j(2,0)]-2*m_cfData->halfFluxAttatched^2*f_j(1,0)*f_j(1,1)+2*m_cfData->halfFluxAttatched*f_(2,1);
		...
		{
		for(int j=0;j<n;j++){
		utilities::cout.DebuggingInfo()<<"pArray["<<a<<"]["<<b<<"]["<<j<<"]="<<(*(pArray+(a*nbrLLs+b)*n+j)).Get()<<std::endl;
		}}}}
		getchar();
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//	
		...
		dcmplx* v)		        //!<	Pointer to the current array of current spinor v coordinates	
	{
		...
		//	first calculate k m_esp[k] (|z|)

		//	Recursively evaluate elementary symmetric polynomials in n variables
		//	m_esp[k] contains the kth degree elementary symmetic polynomial
		...
		double condNo = n*real(m_esp[n-1].Get());
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
		...
		dcmplx* v)		        //!<	Pointer to the current array of current spinor v coordinates
	{
		...
		for(int i=0;i<n;i++){
		for(int j=0;j<n;j++){
		utilities::cout.DebuggingInfo()<<"Y["<<i<<"]["<<j<<"]="<<m_slaterArray[i*n+j].Get()<<std::endl;
		}}
		getchar();
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
		...
		dcmplx* v)	            //!<	Pointer to the current array of current spinor v coordinates
	{
		...
		for(int j=0;j<nbrUp;j++){
		utilities::cout.DebuggingInfo()<<"Y["<<i<<"]["<<j<<"]="<<m_slaterArrayUp[i*nbrUp+j].Get()<<std::endl;
		}}
		getchar();

		for(int i=0;i<nbrDown;i++){
		for(int j=0;j<nbrDown;j++){
		utilities::cout.DebuggingInfo()<<"Y["<<i<<"]["<<j<<"]="<<m_slaterArrayDown[i*nbrDown+j].Get()<<std::endl;
		}}
		getchar();
		...
		return utilities::linearAlgebra::LogDeterminant<U,P>(m_slaterArrayUp,nbrUp)+utilities::linearAlgebra::LogDeterminant<U,P>(m_slaterArrayDown,nbrDown);
	}
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
		...
		dcmplx* v)	            //!<	Pointer to the current array of current spinor v coordinates
	{
		...
		for(int i=0;i<nbrUp;i++){
		for(int j=0;j<nbrUp;j++){
		utilities::cout.DebuggingInfo()<<"Yup["<<i<<"]["<<j<<"]="<<(*(m_slaterArrayUp+i*nbrUp+j)).Get()<<std::endl;
		}}
		getchar();
		...
		for(int i=0;i<nbrDown;i++){
		for(int j=0;j<nbrDown;j++){
		utilities::cout.DebuggingInfo()<<"Ydown["<<i<<"]["<<j<<"]="<<(*(m_slaterArrayDown+i*nbrDown+j)).Get()<<std::endl;
		}}getchar();}
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
		...
		dcmplx* v)		        //!<	Address of spinor v co-ordinate array	
	{
		...
		for(int i=0;i<n;i++){
		for(int j=0;j<n;j++){
		utilities::cout.DebuggingInfo()<<"Y["<<i<<"]["<<j<<"]="<<*(slaterArray+i*n+j)<<std::endl;
		}}
		getchar();
...
class CompositeFermion : public WaveFunction
{
	...
	//		Variables shared between high and low precision	algorithms
	...
	//	Pointers to required class template implementations

	ArbitraryPrecisionCfWavefunctions<dcmplx,double,64> *m_lowPrec;					//!<	complex<double> implementation
	ArbitraryPrecisionCfWavefunctions<mpfCmplx,mpf_t,_PREC_LEVEL1_> *m_precLevel1;	//!<	mpf complex _PREC_LEVEL1_ implementation
	ArbitraryPrecisionCfWavefunctions<mpfCmplx,mpf_t,_PREC_LEVEL2_> *m_precLevel2;	//!<	mpf complex _PREC_LEVEL2_ implementation
	ArbitraryPrecisionCfWavefunctions<mpfCmplx,mpf_t,_PREC_LEVEL3_> *m_precLevel3;	//!<	mpf complex _PREC_LEVEL3_ implementation
	ArbitraryPrecisionCfWavefunctions<mpfCmplx,mpf_t,_PREC_LEVEL4_> *m_precLevel4;	//!<	mpf complex _PREC_LEVEL4_ implementation
	...
	//	Evaluate the selected CF wave function in the sphere geometry
	...
	//	Evaluate the selected CF wave function in the disc geometry//			
	dcmplx EvaluateWfDisc(const int n,dcmplx*) const;
					
	#endif
	////////////////////////////////////////////////////////////////////////////////

	//	Function for condition to increase precision
	bool IncreasePrecCondition(const dcmplx lowPrecResult,const dcmplx highPrecResult,int& maxPrecLevel) const;

	//	Function to generate number of terms (negative effective field case)
	void GeneratePreFactorNumber(const int n,const int nbrLLs,int* nbrEachLL);

	//	Reset the cfLz and LLoccupation variables
