...
//!
//!			\f[ \Phi(L_z^A,Z_A,Z_B) = \int d \phi exp(-2I\pi L_z^A \phi) 
//!         \psi(z_1 exp(2I\pi \phi),\ldots,z_k exp(2I\pi \phi), z_k+1,\ldots z_N) \f]
//!	
...
//!		region of space).
//!
//!			Copyright (C) Ivan D Rodriguez, Simon C Davenport
//!
...
void InitializeIndexes(MersenneTwister* mt, const FQHE::WaveFunctionData* wfData,
                       const FQHE::CompositeFermionData* cfData, dcmplx* sphereCoords, 
                       const int nbrOrbitals);
int DoMetropolisSampling(const GeneralOptions* parameters, MersenneTwister* mt,
                         const MetropOptions* metropolis, FQHE::WaveFunction* wf,
                         dcmplx *sphereCoords, const int nbrOrbitals);
...
int ApplyLowLyingConstraint(const GeneralOptions* parameters, const int nbrSamples,
                            const int nbr, const int nbrFixed, int* listLzA,
                            dcmplx* listCoord, const int nbrOrbitals);
...
void MakeRoughPlot(const GeneralOptions* parameters, const FQHE::WaveFunctionData* wfData,
                   const FQHE::CompositeFermionData* cfData, const utilities::MpiWrapper& mpi);
...
int main(int argc, char *argv[])
{
    ...
    if(parameters.nbrFixed==0)	
    {
        parameters.nbrFixed=(int)(0.5*wfData.nbr);
    }
	...
	if(0 == mpi.m_id)	// FOR THE MASTER NODE
	{
		...
		{
			utilities::cout.MainOutput() << "\tNo. thermalizing Metropolis function calls\t" 
			                             << metropolis.nbrTherms << std::endl;
		...
		else
		{
			...
			utilities::cout.MainOutput() << "\tRho_A dimensions\t\t" << parameters.rows << "x" 
			                             << parameters.columns << std::endl;
...
switch(parameters.choice)
{
	...
	{	
        ...
        if(0 == mpi.m_id)	// FOR THE MASTER NODE
		{
			...
			if(!f_coord.is_open())
			{
				std::cerr << "\tFile: " << GenerateIndexesFileName(&parameters, &cfData, &wfData) 
				          << " not found !!! " << std::endl << std::endl;
				mpi.m_exitFlag = true;
			}
			else
			{
			    utilities::cout.MainOutput() << "\n\tFind indexes file at " 
			        << GenerateIndexesFileName(&parameters, &cfData, &wfData) << std::endl << std::endl;
			}
	    }
	    mpi.ExitFlagTest();
		if(0 == mpi.m_id)	// FOR THE MASTER NODE
		{
			...
			utilities::cout.SecondaryOutput() << "\tLowest lzA EWF state\t\t\t\t" 
			    << FindStartSector(&parameters, &wfData, &cfData) << std::endl;
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//	
	...
	{ 
        ...
        if(0 == mpi.m_id)	// FOR THE MASTER NODE
		{
			...
			{
				std::cerr << "\tThe dimension of rho_A (" << parameters.rows << "," << parameters.columns 
				          << ") exceeds the number of indexes (" << dim1 << ") in " 
				          << GenerateIndexesFileName(&parameters, &cfData, &wfData) << std::endl;
				mpi.m_exitFlag = true;
			}
        }
        mpi.ExitFlagTest();
		...
		{	
			...
			for(int i=0; i<dim1; ++i)
			{
				...
				if(Modulus(i, (int) (dim1/(parameters.rows-1)))==0 && counter1 != parameters.rows - 1)		
				{
				    ++counter1;
				}
				if(Modulus(i, (int) (dim1/(parameters.columns-1)))==0 && counter2 != parameters.columns-1)	
				{
				    ++counter2;
				}
			}
		...
		MPI_Bcast(columnsRho, (parameters.columns+2)*(wfData.nbr-parameters.nbrFixed), 
		          MPI_DOUBLE_COMPLEX, 0, mpi.m_comm);
		MPI_Barrier(mpi.m_comm);
        //////	    CALCULATE FOURIER TRANSFORMS      //////////////////////////
		mpi.DivideTasks(mpi.m_id,parameters.rows, mpi.m_nbrProcs, &mpi.m_firstTask, &mpi.m_lastTask, true);
		MPI_Barrier(mpi.m_comm);
		dcmplx* waveFunctionValue = new dcmplx[parameters.nbrSect];
		dcmplx* sphereCoords = new dcmplx[wfData.nbr];
        std::ofstream f_sector;
		f_sector.open(GenerateNodeFileName(mpi.m_id, &parameters, &cfData, &wfData).c_str(), 
		                                   std::ios::out | std::ios::binary);
        utilities::cout.MainOutput() << "\tCreating output file at " 
                                     << GenerateNodeFileName(mpi.m_id, &parameters, &cfData, &wfData) << std::endl;
        MPI_Barrier(mpi.m_comm);
        ...
        MPI_Barrier(mpi.m_comm);
		utilities::cout.MainOutput() << "\tCompleted output file: "
		                             << GenerateNodeFileName(mpi.m_id, &parameters, &cfData, &wfData) << std::endl;
        MPI_Barrier(mpi.m_comm);
		delete[] rowsRho;	
		delete[] columnsRho;
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\//
	case 3:
	{
		...
		if(0 == mpi.m_id)	// FOR THE MASTER NODE	//
		{
			...
			while(true)
			{
				f_sector.open(GenerateNodeFileName(nbrProcs, &parameters, &cfData, &wfData).c_str(), 
				              std::ios::in | std::ios::binary);
				if(!f_sector.is_open())
				{	 
						utilities::cout.SecondaryOutput() << "\tFile: " << GenerateNodeFileName(nbrProcs, 
						    &parameters, &cfData, &wfData) << " not found !!! " << std::endl << std::endl;
						break;
				}
				else
				{
					utilities::cout.SecondaryOutput() << "\tDetected file: " << GenerateNodeFileName(nbrProcs, 
					    &parameters, &cfData, &wfData) << std::endl;
					++nbrProcs;
					f_sector.close();
				}
			}
            ...
            utilities::cout.SecondaryOutput() << std::endl;
			utilities::cout.SecondaryOutput() << "\tAllocating " << (long int) parameters.rows*parameters.columns*
			    parameters.nbrSect*sizeof(dcmplx)/(1024*1024) << " mb of memory to store data\n" << std::endl;
			...
			{
				f_sector.open(GenerateNodeFileName(proc, &parameters, &cfData, &wfData).c_str(), 
				              std::ios::in | std::ios::binary);
				if(!f_sector.is_open())
				{
						std::cerr << "\tFile: " << GenerateNodeFileName(proc, &parameters, &cfData, &wfData)
						          << " not found !!! " << std::endl << std::endl;
						exit(EXIT_FAILURE);
				}
				...
				f_sector.close();
				utilities::cout.SecondaryOutput() << "\tCompleted reading in file: " 
				    << GenerateNodeFileName(proc, &parameters, &cfData, &wfData) << std::endl;
			}
			utilities::cout.SecondaryOutput() << std::endl;
			for(int k=0; k<parameters.nbrSect; ++k)
			{
				f_sector.open(GenerateSectorFileName(k, &parameters, &cfData, &wfData).c_str(), 
				              std::ios::out | std::ios::binary);
				...
				f_sector.close();
				utilities::cout.SecondaryOutput() << "\tCompleted writing file: " 
				    << GenerateSectorFileName(k, &parameters, &cfData, &wfData) << std::endl;
			}
		...
		if(0 == mpi.m_id)	// FOR THE MASTER NODE
		{
			...
			utilities::cout.SecondaryOutput() << "\n\tRho_A size: \t" << parameters.rows << "x" 
			                                  << parameters.columns << std::endl << std::endl;
			...
			for(int k=0; k<parameters.nbrSect; ++k)
			{
				...
				if(!f_sector.is_open())
				{	 
					std::cerr << "\tFile: " << GenerateSectorFileName(k, &parameters, &cfData, &wfData)
					          << " not found !!! " << std::endl << std::endl;
					exit(EXIT_FAILURE);
				}
				...
				{
					std::cerr << "\tFile: " << GenerateEigenvaluesFileName(k, &parameters, &cfData, &wfData) 
					          << " not found !!! " << std::endl << std::endl;
					exit(EXIT_FAILURE);
				}
    ...
    const int nbrOrbitals)                   //!<    Number of orbitals (defined as 2*monopoleStrength+1)
{
    ...
    else if(wfData->type == FQHE::_BONDERSON_SLINGERLAND_ && abs(cfData->LLup)==2  && cfData->NEF==true && wfData->nbr<=36)
    {
	    ...
	    for (int i=0; i<wfData->nbr; ++i)
	    {
	        std::cout << "initCoeffs[i] = " << initCoeffs[i] << " arg = " 
	                  << ((initCoeffs[i]-wfData->monopoleStrength)/wfData->monopoleStrength) << std::endl;
		    sphereCoords[i] = dcmplx(acos((initCoeffs[i]-wfData->monopoleStrength)/wfData->monopoleStrength), 
		                             Modulus(mt->random(), 2.0*PI)) ;
	    }
    ...
    {
	    //////  Jain 2/3  //////
	    double initCoeffs[40] = {0.1, 1.0, 3.0, 3.8, 6.0, 6.8, 9.0, 9.8, 12.0,  12.8, 15.0, 15.8, 18.0, 18.8, 
	    21.0, 21.8, 24.0, 24.8, 27.0, 27.8, 30.0, 30.8, 33.0, 33.8, 36.0, 36.8, 39.0, 39.8, 42.0, 42.8, 45.0, 
	    45.8, 48.0, 48.8, 51.0, 51.8, 53.0, 53.8, 56.0, 56.8};
	    for (int i=0; i<wfData->nbr; ++i)
	    {
		    sphereCoords[i] = dcmplx(acos((initCoeffs[i]-wfData->monopoleStrength)/wfData->monopoleStrength),
		                             Modulus(mt->random(), 2.0*PI)) ;
    ...
    else
    {
	    ...
	    {
		    f_init >> initCoeff;
		    sphereCoords[i] = dcmplx(acos((initCoeff-wfData->monopoleStrength)/wfData->monopoleStrength),
		                             Modulus(mt->random(), 2.0*PI)) ;
	    }
	    f_init.close();
    }
    ...
    const int nbrOrbitals)            //!<    Number of orbitals (defined as 2*monopoleStrength+1)
{
	...
	utilities::cout.SecondaryOutput() << "\tAccepted " << accepted << "\tRejected " << rejected 
	                                  << "\tTypical log wf value " << parameters->renormFactor*log(10)+oldwfValue << std::endl;
    ...
    dcmplx* listCoord)    //!<	List of co-ordinate samples (of dimension samples*nbr)
{
	...
	for(int i=0; i<nbrSamples-1; ++i)
	{
		...
		for(int j=i+1; j<nbrSamples; ++j)
		{
			...
			for (int k=0; k<parameters->nbrFixed; ++k)
			{
				if(listLzA[i*nbr+k] == listLzA[j*nbr+k] && counter1 != nbrFixed)	
				{
				    ++counter2;
				}
			}
...
//!
//! This function evaluates a fast Fourier transform
//!
    ...
    dcmplx *waveFunctionValue)          //!<   An array to store a set of wave function values for each sector
{
	...
	{ 
		...
		dcmplx wfValue;
		wfValue = wf->EvaluateWfSphere(nbr, u, v);
		wfValue += parameters->renormFactor*log(10);		//	renormalize to avoid overflows
		wfValue = exp(wfValue);			//	exponentiate
		if(std::isnan(real(wfValue)) || std::isinf(real(wfValue)) || real(wfValue)==0)
		{
			std::cerr << "\tWARNING: 0 nan or inf wave function value detected, renorm is " 
			          << parameters-> renormFactor << std::endl;	
			getchar();
		}
